...
export class FootprintLayer extends Layer {
  constructor(options = {}) {
    ...
    this.onFootprintChange = null; // Callback for React integration
  }
  ...
  handleMouseDown(e) {
    ...
    if (this.currentTool === 'rectangle') {
      ...
      // Notify dimension layer immediately
      const dimensionLayer = this.surface.layers.find(l => l.id === 'dimensions');
      if (dimensionLayer) {
        dimensionLayer.setFootprint(this.footprint);
      }
  ...
  handleMouseMove(e) {
    ...
    if (this.isDrawing && this.drawStart && this.footprint) {
      ...
      try {
        ...
        // Update dimension layer to show live dimensions
        const dimensionLayer = this.surface.layers.find(l => l.id === 'dimensions');
        if (dimensionLayer) {
          dimensionLayer.setFootprint(this.footprint);
        }
  ...
  handleMouseUp(e) {
    ...
    if (this.isDrawing) {
      ...
      if (this.footprint.width_ft < 1 || this.footprint.length_ft < 1) {
        ...
        this.footprint = null;
        if (this.onFootprintChange) {
          this.onFootprintChange(null);
        }
        ...
        if (this.onFootprintChange) {
          this.onFootprintChange(this.footprint);
        }
      }
    }
